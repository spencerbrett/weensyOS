...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if(proc_array[current->p_parent].p_state == P_BLOCKED) {
			proc_array[current->p_parent].p_registers.reg_eax = current->p_exit_status;
			proc_array[current->p_parent].p_state = P_RUNNABLE;
		}
		schedule();
	...
	case INT_SYS_WAIT: {
		...
		// * A process that isn't the current process's child.
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			|| current->p_pid != proc_array[p].p_parent )
			current->p_registers.reg_eax = -1;
		else if (proc_array[p].p_state == P_ZOMBIE){
			current->p_registers.reg_eax = proc_array[p].p_exit_status;
			//Reap the zombie
			proc_array[p].p_state = P_EMPTY;
			proc_array[p].p_parent = 0;
		}
		else{
...
do_fork(process_t *parent)
{
	...
	proc_array[pid].p_parent = parent->p_pid;
